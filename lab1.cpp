...
int check_keys(XEvent *e, Game *game)
{
	...
	if (key == XK_b) {
		game->bubbler = !(game->bubbler);
	  	std::cout << "Bubbler button" << std::endl;
	}
...
void movement(Game *game)
{
    ...
    if (game->bubbler) {
         for (int i=0; i<10; i++)
	  	...
	  	std::cout << "bubble test1" << std::endl;
...
void render(Game *game)
{
    //text code for asteroids example in render
    Rect r;
    glClear(GL_COLOR_BUFFER_BIT);
    r.bot = WINDOW_HEIGHT - 200;
    r.left = 10;
    r.center = 0;
    ggprint8b(&r, 16, 0x00ffff00, "cs335 WATERFALL MODEL");
